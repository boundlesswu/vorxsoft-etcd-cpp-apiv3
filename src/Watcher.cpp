...
etcd::Watcher::Watcher(std::string const & address, std::string const & key, watcher_callback callback, void* ptr) {
	std::string stripped_address(address);
	std::string substr("http://");
	std::string::size_type i = stripped_address.find(substr);
	if (i != std::string::npos)
...
void etcd::Watcher::doWatch(std::string const & key, watcher_callback callback, void* ptr)
{
	etcdv3::ActionParameters params;
	params.key.assign(key);
	params.withPrefix = true;
	...
	currentTask = pplx::task<void>([this, callback,ptr]()
	{
		return call->waitForResponse(callback,ptr);
	});
}